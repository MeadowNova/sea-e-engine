...
@dataclass
class PrintSize:
    """Standard print size specification at optimized DPI."""
    name: str
    width_inches: float
    height_inches: float
    width_px: int
...
class JPEGResizer:
    ...
    # Standard print sizes (optimized for JPEG quality/size balance)
    STANDARD_SIZES = {
        "24x36": PrintSize(
            name="24x36",
            width_inches=24.0,
            height_inches=36.0,
            width_px=3600,  # 150 DPI - excellent for large prints
            height_px=5400,
        ...
        "18x24": PrintSize(
            ...
            width_inches=18.0,
            height_inches=24.0,
            width_px=3600,  # 200 DPI - great print quality
            height_px=4800,
    ...
    def create_print_ready_jpegs(self, png_path: str, design_name: str) -> JPEGResizeResult:
        ...
        try:
            ...
            if not png_path.exists():
                raise FileNotFoundError(f"Source file not found: {png_path}")
    ...
    def _create_high_quality_resize(self, source_img: Image.Image, 
        ...
        # For print-ready files, we want the artwork to fill the entire canvas
        # This provides maximum quality and no wasted white space
        # Simply resize to exact target dimensions using high-quality LANCZOS resampling
        ...
        resized = enhancer.enhance(1.05)  # Slight sharpening for JPEG
    ...
    def get_size_info(self) -> Dict[str, Dict]:
        ...
        return {
            name: {
                ...
                'estimated_dpi': f"{spec.width_px/spec.width_inches:.0f} DPI",
